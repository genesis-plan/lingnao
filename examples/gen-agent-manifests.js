...
`;
    ...
    fs.writeFileSync(path.join(ROOT, 'openai-tools.json'), JSON.stringify(openai, null, 2), 'utf8');
    fs.writeFileSync(path.join(ROOT, 'anthropic-tools.json'), JSON.stringify(anthropic, null, 2), 'utf8');
    fs.writeFileSync(path.join(ROOT, 'llms.txt'), llms, 'utf8');

    console.log('OK 已生成接入清单（工具数 ' + n + '）');
    console.log('  llms.txt             ' + fs.statSync(path.join(ROOT, 'llms.txt')).size + ' bytes');
    console.log('  openai-tools.json    ' + fs.statSync(path.join(ROOT, 'openai-tools.json')).size + ' bytes');
    console.log('  anthropic-tools.json ' + fs.statSync(path.join(ROOT, 'anthropic-tools.json')).size + ' bytes');
  }).catch(e => {
