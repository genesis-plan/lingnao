...
LingJing Brain is a **pure cognitive engine**: perception → world graph → reasoning → audit → metacognition.
...
```bash
# kernel syntax check
node -e "const fs=require('fs');const h=fs.readFileSync('灵境.html','utf8');\
...
Kernel: `灵境.html` (~2,400 lines, single source of truth; UMD is extracted from it).
