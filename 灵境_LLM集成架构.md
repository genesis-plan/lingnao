# 灵境 LingJing · 免费 LLM 集成架构（感知层 + 解释层）

> 本文档说明「一天 50 次」的 OpenRouter 免费模型如何真正用于完善灵境产品：
...
## 2. 对应代码（文件：灵境.html 内核；lingjing-mcp.js 服务）

| 函数 | 位置 | 作用 | 真消费的数学/资料 |
|---|---|---|---|
| `perceiveLLM(text, key)` | 灵境.html ~L569 | NL→JSON 感知 | 感知层（文档 Layer1） |
| `reason()` 主推理 IMA 注入 | 灵境.html ~L253 | 未知/低置信时检索 IMA 证据 | ima_378/365 |
| `explainWithLLM(result, key)` | 灵境.html ~L602 | 推理结果+IMA→中文 | 把 `imaKnowledge.get(id)` 标题作为引用资料喂给模型 |
| `askBrain(text, key)` | 灵境.html ~L622 | 端到端编排：感知→推理→解释 | 上述三者 |
| `askBrainLogic` / `explainLogic` | lingjing-mcp.js | MCP 工具后端 | 同上 |
| MCP 工具 `ask` / `explain` | lingjing-mcp.js TOOLS | 暴露给智能体调用 | 同上 |
...
- 免费 LLM 是灵境运行时的「耳朵（理解人话）+ 嘴巴（讲人话）」，**不是**代码生成器或架构设计器；确定性推理、可审计证明、IMA 数学仍由内核完成。
