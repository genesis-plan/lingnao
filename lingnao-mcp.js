...
const TOOLS = [
  ...
  {
    name: 'ask', description: '用大白话让大脑理解并规划：免费 LLM 把自然语言理解为结构化目标 → 灵脑确定性可审计推理 → 免费 LLM 把结果+IMA 资料用中文解释。返回 percept/reason/explanation。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'ima_load', description: '加载用户 IMA 数学库知识壳（默认同目录 ima_knowledge.json，431 条公理/定理/定义/方法/思想方法）注入灵脑内核：KB 可检索、审计报告可引用证据、元认知层可路由。可传 {path} 指定文件，或直传 {entries:[...]}。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'positioning', description: '返回灵脑产品定位 POSITIONING（具身智能在物理世界干活的通用大脑）+ 当前注册身体 BODY。让外部智能体确认大脑的自我定位与边界。',
    inputSchema: { type: 'object', properties: {}, required: [] },
  },
];
