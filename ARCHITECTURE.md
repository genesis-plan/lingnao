# 灵境 LingJing（世界通用大脑）架构白皮书（WorldBrain Architecture）

> 本文档把灵境作为**一个本地确定性、可审计的推理大模型**来定义，而非普通工具脚本。
...
**灵境 = 可审计推理大模型**：内核把「世界图 → A\* 可审计推理 → 物理载体执行 → 学习闭环」封装为单一确定性引擎，对外以 **MCP stdio / 网页演示** 两种接口暴露。
