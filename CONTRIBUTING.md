...
感谢你关注「灵境 LingJing（世界通用大脑）」。本仓库定位为**可审计、确定性、不幻觉**的推理大模型内核，欢迎外部测试、反馈与代码贡献。

## 一、先跑通自测（验证不幻觉）
```bash
git clone https://github.com/genesis-plan/lingjing && cd lingjing
node lingjing-mcp.js --selftest
...
| 类型 | 入口 | 说明 |
|---|---|---|
| Bug | Issue 模板 `bug` | 附复现步骤 + `node lingjing-mcp.js --selftest` 完整输出 |
...
3. 改动后跑：`node lingjing-mcp.js --selftest` 必须全绿。
4. 遵守下方「诚实边界」约定。
...
## 五、代码组织
- `灵境.html`：单文件演示 + **内控内核**（唯一真源，MCP 用 vm 抽取复用）。
- `lingjing-mcp.js`：MCP stdio 服务，从 `灵境.html` 抽取内核，勿在 MCP 内重复实现内核逻辑。
- 内核标记：`// ==KERNEL START==` … `// ==KERNEL END==`；改动内核请在 `灵境.html` 内，MCP 自动继承。
